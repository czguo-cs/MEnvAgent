...
async def fetch_repo_size(
    session: ClientSession, repo_url: str, token_cycle, console
) -> int:
    """
    Fetch the total size of a repository using GitHub API (in KB).
    
    Args:
        session: aiohttp client session.
        repo_url: Repository API URL (e.g., 'https://api.github.com/repos/owner/repo').
    
    Returns:
        Repository size (KB), returns 0 if fetch fails.
    """
    # Repository info API path is the repo_url itself
    repo_data = await fetch_json(session, repo_url, token_cycle, console)
    
    # 'size' field represents repository size in Kilobytes (KB)
    # Default value 0 means fetch failed or repository size is 0
    size_kb = repo_data.get("size", 0)
    
    return size_kb

async def fetch_pull_count_from_link_header(
    ...
    while True:
        ...
        try:
            async with session.get(
                ...
                # print(link)
                if not link:
                    # print(f"[yellow]No Link header found for {url}[/yellow]")
                    return len(await resp.json())  # maybe 0 or 1 item
                match = re.search(r'&page=(\d+)>; rel="last"', link)
                # print(f"Last page: {match.group(1)}")
                return int(match.group(1)) if match else 0
        except Exception as e:
            console.log(f"[red]Error fetching count from {url}: {e}[/red]")
            return 0

# Function name clarified to indicate it fetches Issue count
async def fetch_issue_count_from_search_api(
    session: ClientSession, repo_url: str, token_cycle, console
) -> int:
    """
    Fetch the total Issue count (open and closed) using GitHub Search API.
    """
    # 1. Extract 'owner/repo' from repository URL
    # Example: 'https://api.github.com/repos/octocat/Spoon-Knife' -> 'octocat/Spoon-Knife'
    try:
        owner_repo = "/".join(repo_url.split("/")[-2:])
    except IndexError:
        console.log(f"[red]Invalid repository URL format: {repo_url}[/red]")
        return 0

    # 2. Construct Search API URL and parameters
    # Search 'type:issue' in 'repo:owner/repo'
    search_url = "https://api.github.com/search/issues"
    query_params = {
        "q": f"repo:{owner_repo} type:issue",
        "per_page": 1, # Only need one result to get total count
    }
    ...
    while True:
        ...
        try:
            async with session.get(
                ...
                if resp.status == 403:
                    # Rate limit reached, wait 1 second and try next token
                    await asyncio.sleep(1)
                    continue
                ...
                # Search API response contains 'total_count' field
                total_count = data.get("total_count", 0)
                
                # console.log(f"[green]Successfully fetched issue count for {owner_repo}: {total_count}[/green]")
                return total_count

        except Exception as e:
            # Catch network, JSON parsing and other errors
...
async def filter_repo(
    ...
    if forks < criteria["min_forks"]:
        progress.update(task_id, advance=1)
        # print(f"[red]Skipping {full_name} due to low forks count: {forks}[/red]")
        return None

    owner_repo_url = f"{GITHUB_API}/repos/{full_name}"
    issues_url = f"{owner_repo_url}"
    ...
    issues_count, pulls_count, langs_data = await asyncio.gather(
        fetch_issue_count_from_search_api(session, issues_url, token_cycle, console),
        fetch_pull_count_from_link_header(session, pulls_url, token_cycle, console),
        fetch_json(session, langs_url, token_cycle, console),
    )
    print(f"{full_name}: Issues: {issues_count}, Pulls: {pulls_count}")

    # total = issues_count + pulls_count
    # if total < criteria["min_total_pr_issues"]:
    #     progress.update(task_id, advance=1)
    #     return None
    if issues_count < criteria["min_issues"] or pulls_count < criteria["min_pr"]:
        progress.update(task_id, advance=1)
        return None
