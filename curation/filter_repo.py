...
async def fetch_repo_size(
    ...
    """
    使用 GitHub API 获取仓库的总大小（单位：KB）。
    
    Args:
        session: aiohttp 客户端会话。
        repo_url: 仓库的 API URL (e.g., 'https://api.github.com/repos/owner/repo')。
    
    Returns:
        仓库大小（KB），如果获取失败则返回 0。
    """
    # 仓库信息 API 路径就是 repo_url 本身
    repo_data = await fetch_json(session, repo_url, token_cycle, console)
    
    # 'size' 字段表示仓库大小，单位为 Kilobytes (KB)
    # 默认值 0，表示获取失败或仓库大小为 0
...
# 函数名称已修改，明确它用于获取 Issue 数量
async def fetch_issue_count_from_search_api(
    session: ClientSession, repo_url: str, token_cycle, console
) -> int:
    """
    使用 GitHub 搜索 API 获取仓库的总 Issue 数量（包括开放和关闭）。
    """
    # 1. 从仓库 URL 中提取 'owner/repo'
    # 示例: 'https://api.github.com/repos/octocat/Spoon-Knife' -> 'octocat/Spoon-Knife'
    try:
    ...
    # 2. 构造搜索 API 的 URL 和参数
    # 搜索 'repo:owner/repo' 中 'type:issue' 的所有项目
    search_url = "https://api.github.com/search/issues"
    query_params = {
        "q": f"repo:{owner_repo} type:issue",
        "per_page": 1, # 只需要一个结果来获取总数
    }
...
async def filter_repo(
    ...
    if forks < criteria["min_forks"]:
        ...
        return None
    if criteria["max_forks"] > criteria["min_forks"] and forks >= criteria["max_forks"]:
        progress.update(task_id, advance=1)
        # print(f"[red]Skipping {full_name} due to high forks count: {forks}[/red]")
        return None
    # if criteria["max_forks"] > criteria["min_forks"]:
    #     if forks < criteria["min_forks"] or forks >= criteria["max_forks"]:
    #         progress.update(task_id, advance=1)
    #         # print(f"[red]Skipping {full_name} due to low forks count: {forks}[/red]")
    #         return None
    # else: 
    ...
    # )
    (langs_data,) = await asyncio.gather(
        fetch_json(session, langs_url, token_cycle, console),
    )
    issues_count = 10000
    pulls_count = 10000
