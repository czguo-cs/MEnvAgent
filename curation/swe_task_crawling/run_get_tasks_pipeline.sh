...
usage() {
    ...
    echo "  --pull-number    Specific pull request number to process (optional)"
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    for token_id in "${TOKEN_IDS[@]}"; do
        ...
        echo "    --max_cutoff_date $MAX_CUTOFF_DATE \\"
        if [[ -n "$MAX_PULLS" ]]; then
...
start_crawling() {
    ...
    cmd="$cmd --max_cutoff_date $MAX_CUTOFF_DATE"
    ...
    if [[ -n "$PULL_NUMBER" ]]; then
        cmd="$cmd --pull_number $PULL_NUMBER"
    fi
