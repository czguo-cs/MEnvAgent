import requests
import os
import json
from datetime import datetime
import time
import logging

logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s"
)
logger = logging.getLogger(__name__)

def run_graphql_query(query, token):
    def call_api(query, token):
        request = requests.post('https://api.github.com/graphql', json={'query': query}, headers={
            'Authorization': f'Bearer {token}',
            'Content-Type': 'application/json'
        })
        if request.status_code == 200:
            response_json = request.json()
            rl = int(request.headers.get('x-ratelimit-remaining'))
            if rl < 1000:
                print(f"❗️ GraphQL Rate limit remaining: {rl}")
            if 'data' in response_json:
                return response_json
            else:
                raise Exception(f"GraphQl Query failed to return data: {response_json}")
        elif request.status_code == 403:
            rl = request.headers.get('x-ratelimit-remaining')
            logger.error(f"GraphQL Rate limit remaining: {rl}")
            # Rate limit exceeded
            if rl == 0:
                reset_time = int(request.headers.get('x-ratelimit-reset'))
                sleep_time = max(0, reset_time - time.time()) + 1
            else:
                sleep_time = 60 * 5
            logger.error(f"Sleeping for {sleep_time} seconds")
            time.sleep(sleep_time)
            return call_api(query, token)
        else:
            raise Exception(f"GraphQL Query failed to run with status code {request.status_code}. {request.json()}")

    for i in range(3):
        try:
            return call_api(query, token)
        except requests.exceptions.RequestException as e:
            logger.error(f"Request failed: {e}")
            time.sleep(60)  # Wait for 1 minute before retrying
    logger.error("max retry reached, giving up...")
    return {}

def get_closed_issue_events(owner, repo_name, token):
    query_template = """
    {{
      repository(owner: "{owner}", name: "{repo_name}") {{
        issues(states: CLOSED, first: 100, after: {after}) {{
          pageInfo {{
            endCursor
            hasNextPage
          }}
          edges {{
            node {{
              number
              title
              body
              timelineItems(last: 100) {{
                nodes {{
                  __typename
                  ... on ClosedEvent {{
                    actor {{
                      login
                    }}
                    createdAt
                    closer {{
                      __typename
                      ... on PullRequest {{
                        number
                        title
                      }}
                    }}
                  }}
                }}
              }}
            }}
          }}
        }}
      }}
    }}
    """

    after_cursor = "null"
    issues = []

    while True:
        query = query_template.format(owner=owner, repo_name=repo_name, after=after_cursor)
        result = run_graphql_query(query, token)
        if not result:
            break
        issues_data = result['data']['repository']['issues']
        issues.extend(issues_data['edges'])

        if issues_data['pageInfo']['hasNextPage']:
            after_cursor = f'"{issues_data["pageInfo"]["endCursor"]}"'
        else:
            break

    return issues

def collect_single_pull(owner: str,
                       repo_name: str,
                       token: str,
                       path_issue: str,
                       pull_number: int):
    """Collect data for a single pull request"""
    query = """
    query {
      repository(owner: "%s", name: "%s") {
        pullRequest(number: %d) {
          closingIssuesReferences(first: 100) {
            nodes {
              number
            }
          }
        }
      }
    }
    """ % (owner, repo_name, pull_number)
    
    result = run_graphql_query(query, token)
    if not result or not result.get('data'):
        return
        
    pr_data = result['data']['repository']['pullRequest']
    if not pr_data or not pr_data['closingIssuesReferences']['nodes']:
        return
        
    for issue in pr_data['closingIssuesReferences']['nodes']:
        instance = {
            "repo": f"{owner}/{repo_name}",
            "pull_number": pull_number,
            "issue_number": issue['number'],
        }
        with open(path_issue, "a") as f:
            f.write(json.dumps(instance) + "\n")

def collect_closed_issues(owner: str,
                          repo_name: str,
                          token: str,
                          path_issue: str,
                          cutoff_date: str,
                          max_cutoff_date: str):
    # Parse cutoff date
    cutoff_date = datetime.strptime(cutoff_date, "%Y%m%d")
    max_cutoff_date = datetime.strptime(max_cutoff_date, "%Y%m%d")
    # Read existing issues to avoid duplicates
    existing_issues = set()
    if os.path.exists(path_issue):
        with open(path_issue, "r") as f:
            for line in f:
                instance = json.loads(line)
                existing_issues.add(instance["issue_number"])

    # Fetch closed issues
    closed_issues = get_closed_issue_events(owner, repo_name, token)

    for issue in closed_issues:
        issue_node = issue['node']
        issue_number = issue_node['number']
        # issue_title = issue_node['title']
        # issue_body = issue_node['body']

        if issue_number in existing_issues:
            continue

        for event in issue_node['timelineItems']['nodes']:
            if event['__typename'] == 'ClosedEvent' and event.get('closer') and event['closer']['__typename'] == 'PullRequest':
                pull_number = event['closer']['number']
                closed_at = datetime.strptime(event['createdAt'], "%Y-%m-%dT%H:%M:%SZ")

                if closed_at >= cutoff_date and closed_at <= max_cutoff_date:
                    instance = {
                        "repo": f"{owner}/{repo_name}",
                        "pull_number": pull_number,
                        "issue_number": issue_number,
                    }

                    with open(path_issue, "a") as f:
                        f.write(json.dumps(instance) + "\n")

def fetch_pulls(repo: str,
                token: str,
                path_pulls: str,
                cutoff_date: str,
                max_cutoff_date: str,
                pull_number: int = None):
    owner, repo_name = repo.split("/")
    path_issue = os.path.join(path_pulls, f"{repo_name}-issues-{cutoff_date}-{max_cutoff_date}.jsonl")
    path_pull2issue = os.path.join(path_pulls, f"{repo_name}-pull2issue-{cutoff_date}-{max_cutoff_date}.jsonl")

    logger.info(f"[{repo}]  Fetching closed issues ...")
    if pull_number:
        # Special handling for single pull request
        collect_single_pull(owner, repo_name, token, path_issue, pull_number)
    else:
        collect_closed_issues(owner, repo_name, token, path_issue, cutoff_date,max_cutoff_date)

    if not os.path.exists(path_issue):
        logger.info(f"[{repo}] ❌ Found no issues closed by a pull.")
        # Create a fake empty file
        with open(path_pull2issue, "w") as f:
            f.write("")
    else:
    # Post-process the data to create a mapping from pull to issue
        pull2issue = dict()
        with open(path_issue, "r") as f:
            for line in f:
                instance = json.loads(line)
                if instance["pull_number"] not in pull2issue:
                    pull2issue[instance["pull_number"]] = [instance["issue_number"]]
                else:
                    pull2issue[instance["pull_number"]].append(instance["issue_number"])
        with open(path_pull2issue, "w") as f:
            for key, value in pull2issue.items():
                f.write(json.dumps({"pull": key, "issue": value}) + "\n")
        logger.info(f"[{repo}] ⭕️ Successfully saved {len(pull2issue)} closed issue data instances.")


if __name__ == "__main__":

    gh_token = os.environ.get('GH_TOKEN')

    # Specify the repository owner and name
    owner = "huggingface"
    repo_name = "transformers"

    # Fetch pulls with cutoff date filter
    cutoff_date = "20180101"
    fetch_pulls("keras-team/keras-contrib", gh_token, ".", cutoff_date)
