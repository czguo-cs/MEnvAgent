...
def construct_data_files(data: dict):
    ...
    """
    repos, language,path_prs, path_tasks, max_pulls, cutoff_date,max_cutoff_date,token = (
        data["repos"],
        data["language"],
        data["path_prs"],
        data["path_tasks"],
    ...
    for index,repo in enumerate(repos):
        ...
        try:
            ...
            if cutoff_date and max_cutoff_date:
                path_pr = path_pr.replace(".jsonl", f"-{cutoff_date}-{max_cutoff_date}.jsonl")

            assert issue_first, "Only issue-first approach is supported now"
            # Get pull2issue
            path_pull2issue = os.path.join(path_prs, f"{repo_name}-pull2issue-{cutoff_date}-{max_cutoff_date}.jsonl")
            if os.path.exists(path_pull2issue):
                ...
                fetch_pulls(repo, token, path_prs, cutoff_date, max_cutoff_date,pull_number)
            ...
            build_dataset(path_pr, path_task, language[index],token)
...
def main(
    ...
    data_pooled = [
        {
            "repos": repos,
            "language": [repo2languages[repo] for repo in repos],
            "path_prs": path_prs,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pull_number",
        type=int,
        help="Specific pull request number to process",
        default=None,
    )
